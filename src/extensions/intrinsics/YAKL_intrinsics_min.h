...
///////////////////////////////////////////////////////////////////////
// Min(left,right) with Array LHS and scalar RHS
///////////////////////////////////////////////////////////////////////
...
template <
    ...
    typename std::enable_if<std::is_arithmetic<T2>::value, bool>::type = false>
inline Array<decltype(T1()), N, memDevice, STYLE>
min(Array<T1, N, memDevice, STYLE> const &left, T2 const &right) {
  return min_array_scalar(left, right);
}
template <
    class T1, class T2, int N, size_t D0, size_t D1, size_t D2, size_t D3,
    typename std::enable_if<std::is_arithmetic<T2>::value, bool>::type = false>
KOKKOS_INLINE_FUNCTION SArray<decltype(T1()), N, D0, D1, D2, D3>
min(SArray<T1, N, D0, D1, D2, D3> const &left, T2 const &right) {
...
min(FSArray<T1, N, B0, B1, B2, B3> const &left, T2 const &right) {
...
///////////////////////////////////////////////////////////////////////
// Min(left,right) with scalar LHS and Array RHS
///////////////////////////////////////////////////////////////////////
...
min(T1 const &left, Array<T2, N, memHost, STYLE> const &right) {
  auto ret = right.template createHostObject<decltype(T2())>();
  for (size_t i = 0; i < ret.totElems(); i++) {
    ret.data()[i] = std::min(left, right.data()[i]);
  }
  return ret;
}
template <class T1, class T2, int N, int STYLE>
inline Array<decltype(T2()), N, memDevice, STYLE>
min_scalar_array(T1 const &left, Array<T2, N, memDevice, STYLE> const &right) {
...
template <
    ...
    typename std::enable_if<std::is_arithmetic<T1>::value, bool>::type = false>
inline Array<decltype(T2()), N, memDevice, STYLE>
min(T1 const &left, Array<T2, N, memDevice, STYLE> const &right) {
  return min_scalar_array(left, right);
}
template <
    class T1, class T2, int N, size_t D0, size_t D1, size_t D2, size_t D3,
    typename std::enable_if<std::is_arithmetic<T1>::value, bool>::type = false>
KOKKOS_INLINE_FUNCTION SArray<decltype(T2()), N, D0, D1, D2, D3>
min(T1 const &left, SArray<T2, N, D0, D1, D2, D3> const &right) {
...
min(T1 const &left, FSArray<T2, N, B0, B1, B2, B3> const &right) {
